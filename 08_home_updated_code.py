...
Keys (click video window first):
  ...
  Z/X         → Zoom in/out
  SPACE       → Stop
  H           → Go to ONVIF factory home (camera default preset)
  G           → Save current position as custom home (ONVIF Preset 1)
  J           → Go to custom home (Preset 1 — your saved runway position)
  P           → Print current PTZ position (debugging)
...
Automatic behaviors:
  - If a tracked target is lost for LOST_HOME_SECONDS (default 10s),
    the camera automatically returns to custom home (Preset 1).
    Tracking remains enabled.
  - Manual H press is the only way to reach ONVIF factory home.
...
YOLO_MODEL     = os.getenv("YOLO_MODEL", "/home/anuragmishra/Anurag/Flight_detection/files_7/yolo26m.engine")
...
TRACKER_COAST_FRAMES = int(os.getenv("TRACKER_COAST_FRAMES", "150"))
...
def _save_preset(token, preset_token="1", preset_name="CustomHome"):
    """Save current PTZ position as an ONVIF preset."""
    resp = soap(f"""<SetPreset xmlns="http://www.onvif.org/ver20/ptz/wsdl">
  <ProfileToken>{token}</ProfileToken>
...
def _goto_preset(token, preset_token="1"):
    """Recall a saved ONVIF preset."""
    resp = soap(f"""<GotoPreset xmlns="http://www.onvif.org/ver20/ptz/wsdl">
  <ProfileToken>{token}</ProfileToken>
  <PresetToken>{preset_token}</PresetToken>
  <Speed>
    <PanTilt xmlns="http://www.onvif.org/ver10/schema" x="0.5" y="0.5"/>
    ...
    def run(self):
        while self.running:
            ...
            if action == 'move':
                ...
                # Save current PTZ position as ONVIF preset
                success = _save_preset(self.token, preset_token="1", preset_name="CustomHome")
                if success:
                    print("✅ Custom home preset saved")
                else:
                    print("❌ Failed to save custom home preset")
            elif action == 'goto_preset':
                # Recall saved preset
                success = _goto_preset(self.token, preset_token="1")
                if not success:
                    print("❌ Failed to goto custom home preset")
            elif action == 'custom_home':
                # Legacy: Move via AbsoluteMove to a user-configured position.
                # Kept for backward compatibility with home.json workflow.
...
TRACK_SPD_MAX    = float(os.getenv("TRACK_SPD_MAX", "0.30"))   # HALF the old 0.35 — main fix for oscillation
...
# ── HUD ───────────────────────────────────────────────────────────────────────
def draw_hud(frame, status, mode, bottle_found, fps, track_id=None):
    h, w = frame.shape[:2]
    ...
    # Bottle indicator (track mode) — show committed ID when locked
    if mode == "TRACK":
        dc = (0, 255, 0) if bottle_found else (0, 0, 255)
        # Show ID next to LOCKED when we have one — gives instant visual
        # feedback whether the tracker is holding identity across frames.
        if bottle_found and track_id is not None:
            dl = f"LOCKED #{track_id}"
        elif bottle_found:
            dl = "LOCKED"
        else:
            dl = "SEARCHING"
...
def main():
    ...
    model = YOLO(YOLO_MODEL, task='detect')  # Add task= to suppress warning
    ...
    last_track_id  = None    # int or None — the track ID matching last_box_data (for HUD display)
    stale_counter  = 0
    ...
    while True:
        ...
        if track_mode:
            ...
            try:
                ...
                if chosen is not None:
                    ...
                    # logic below doesn't change. We keep the ID in a
                    # parallel variable so the on-screen HUD can show it.
                    last_box_data = chosen[:5]
                    last_track_id = chosen[5]  # int or None (None = tentative)
                    stale_counter = 0
                else:
        ...
        if track_mode:
            ...
            if last_box_data:
                ...
                # Draw box + line to crosshair. The track ID (or "?" if BoT-SORT
                # hasn't confirmed yet) shows next to the conf so you can see
                # at a glance whether identity is being maintained across frames.
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                id_text = f"#{last_track_id}" if last_track_id is not None else "#?"
                cv2.putText(frame, f"PERSON {id_text}  {conf:.2f}", (x1, y1 - 8),
            ...
            if lost_home_timer.consume_home_signal():
                print(f"⏰ No target for {LOST_HOME_SECONDS}s — returning to custom home preset.")
                # Auto-home ALWAYS goes to custom home preset (J behavior),
                # never ONVIF factory home. Manual H press is the only way
                # to reach ONVIF home.
                ptz_thread.send({'action': 'goto_preset'})
                status = "AUTO HOME (preset)"
        ...
        draw_hud(frame, status, "TRACK" if track_mode else "MANUAL",
                 bottle_found, fps, track_id=tracker_state.committed_id)
        ...
        if key == ord('q'):
            ...
            print("🏠 Going to ONVIF default home")

        elif key == ord('g'):
            # G = Save current PTZ position as ONVIF Preset 1 ("CustomHome")
            # This is NON-BLOCKING — the PTZ thread handles it asynchronously.
            # No JSON files, no get_ptz_position() call, no main-thread delay.
            ptz_thread.send({'action': 'save_preset'})
            status = "SAVING CUSTOM HOME..."
            print("💾 Saving current position as custom home preset")

        elif key == ord('j'):
            # J = Go to custom home (Preset 1)
            track_mode     = False
            last_box_data  = None
            last_track_id  = None
            last_track_cmd = (0.0, 0.0, 0.0)
            zoom_pulse_until = 0.0
            current_action = 'goto_preset'
            status         = "CUSTOM HOME"
            # Scene change invalidates IDs
            tracker_state.reset()
            yolo_thread.reset_tracker()
            lost_home_timer.reset()
            ptz_thread.send({'action': 'goto_preset'})
            print("🏁 Going to custom home preset")
