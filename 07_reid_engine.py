...
TRACK_SPD = float(os.getenv("TRACK_SPEED", "0.20"))
DEAD_ZONE = float(os.getenv("DEAD_ZONE", "0.08"))   # fraction of frame — no move inside this radius. Widened from 0.10 to reduce over-correction near center.
...
TRACK_SPD_MIN    = float(os.getenv("TRACK_SPD_MIN", "0.08"))   # gentler than old default (was 0.08)
TRACK_SPD_MAX    = float(os.getenv("TRACK_SPD_MAX", "0.20"))   # HALF the old 0.35 — main fix for oscillation
...
def main():
    ...
    if not YOLO_MODEL.endswith('.engine'):
        model.to(device)  # Only call .to() for .pt files
    # Warm up
