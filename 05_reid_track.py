...
TRACK_SPD = float(os.getenv("TRACK_SPEED", "0.70"))
DEAD_ZONE = float(os.getenv("DEAD_ZONE", "0.15"))   # fraction of frame — no move inside this radius. Widened from 0.10 to reduce over-correction near center.
...
TRACKER_COAST_FRAMES = int(os.getenv("TRACKER_COAST_FRAMES", "90"))
...
class YOLOThread(threading.Thread):
    ...
    def run(self):
        while self.running:
            ...
            # model.track() with persist=True keeps the tracker's internal
            # Kalman + ID assignment state alive across calls. Without persist,
            # every frame starts from scratch and we'd never get stable IDs.
            #
            # imgsz=640 (configurable) is the single biggest latency win:
            # YOLO internally letterboxes to a square, so a 1920x1080 input
            # at imgsz=1280 takes ~4x the compute of imgsz=640. The detections
            # are returned in the original frame's coordinate space regardless.
            #
            # The result objects' .id field is None until the tracker has
            # enough evidence to confirm a track (typically 1–3 frames).
            # We pass that through verbatim — tracker_state.pick_track()
            # knows how to handle None IDs.
            ...
            boxes_out = []
            r0 = results[0]
            # boxes.id is a tensor or None depending on whether any tracks
            # are confirmed in this frame. Normalize both shapes here.
    ...
    def reset_tracker(self):
        ...
        Safe to call before the first model.track() — guards against the
        attribute chain not existing yet.
        """
        try:
            ...
            # Reset is best-effort — if Ultralytics changes the internals
            # in a future release we don't want this to kill tracking.
            print(f"⚠️  Tracker internal reset skipped: {e}")
...
TRACK_SPD_MAX    = float(os.getenv("TRACK_SPD_MAX", "0.50"))   # HALF the old 0.35 — main fix for oscillation
...
def main():
    ...
    last_track_id  = None    # int or None — the BoT-SORT track ID matching last_box_data (purely for display; steering uses last_box_data)
    stale_counter  = 0
        ...
        if track_mode:
            ...
            try:
                ...
                if chosen is not None:
                    ...
                    last_track_id = chosen[5]  # int or None (None = BoT-SORT tentative)
                    stale_counter = 0
                else:
